...
const LAUNCH_BAT := "C:\\Projects_Dev\\ML_Godot\\Neat_Dino_Main\\python\\.venv\\launch_tcp_server.bat"
...
func _connect_to_server(ip: String, port: int) -> void:
	...
	if err != OK:
		print("[X] TCP: connect_to_host() failed. Code:", err)
		return
...
func send_json(data: Dictionary):
	if not connected:
		print("!!! TCP not connected. Cannot send.")
		return
